...
// mobile 151:395) and Section 4's first two sub-blocks (核心介面優化與服務創新設計
// / 01 路線搜尋與規劃, Figma desktop 127:142 / mobile 147:140; and 02 捷伴陪同服務,
// Figma desktop 127:208 / mobile 147:174) are implemented. Sections 3 and 4
// both reuse the `process` row's content (same row that also carries the
// long-form process narrative in `items`, left untouched) -- each
// component only reads the fields it needs off that same content object
// (StrategyBeforeApp: `strategies`, `visionText`, `mobileVisionText`,
// `beforeAppSubtext`; InterfaceRouteSearch: `interfaceEyebrow`,
// `interfaceHeading`, `interfaceSectionNumber`, `interfaceSectionTitle`,
// `interfaceBlocks`, and the four `interface*MediaUrl` fields;
// InterfaceCompanionService: `companionSectionNumber`,
// `companionSectionTitle`, `companionBlocks`, and the six
// `companion*MediaUrl` fields). Any further sub-block (Figma "03" and
// beyond) follows the same pattern and slots in here in order, each wired
// the same way (fetch content below, pass as props) -- note the Interface
